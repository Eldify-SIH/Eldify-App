 ...
 */
    ...
    fun createPill(pill: Pills): Long {
        ...
        values.put(KEY_PILLNAME, pill.pillId)
    ...
    fun createAlarm(alarm: MedicineAlarm, pill_id: Long): LongArray {
        ...
        var arrayPos = 0
        for (day in alarm.dayOfWeek) {
            if (day) {
                val values = ContentValues()
                values.put(KEY_HOUR, alarm.hour)
                values.put(KEY_MINUTE, alarm.minute)
                values.put(KEY_DAY_WEEK, arrayPos + 1)
                values.put(KEY_ALARMS_PILL_NAME, alarm.pillName)
                values.put(KEY_DOSE_QUANTITY, alarm.doseQuantity)
                values.put(KEY_DOSE_UNITS, alarm.doseUnit)
                values.put(KEY_DATE_STRING, alarm.dateString)
                values.put(KEY_ALARM_ID, alarm.alarmId)
                /** Insert row  */
                val alarm_id = db.insert(ALARM_TABLE, null, values)
    ...
    fun createHistory(history: History) {
        ...
        val values = ContentValues()
        values.put(KEY_PILLNAME, history.pillName)
        values.put(KEY_DATE_STRING, history.dateString)
        values.put(KEY_HOUR, history.hourTaken)
        values.put(KEY_MINUTE, history.minuteTaken)
        values.put(KEY_DOSE_QUANTITY, history.doseQuantity)
        values.put(KEY_DOSE_UNITS, history.doseUnit)
        values.put(KEY_ACTION, history.action)
        values.put(KEY_ALARM_ID, history.alarmId)
        /** Insert row  */
    ...
    fun getPillByName(pillName: String): Pills {
        ...
        val dbPill = ("select * from "
                ...
                + KEY_PILLNAME + " = "
                + "'" + pillName + "'")
        val c = db.rawQuery(dbPill, null)
        val pill = Pills()
        if (c.moveToFirst() && c.count >= 1) {
            pill.pillName = c.getString(c.getColumnIndex(KEY_PILLNAME))
            pill.pillId = c.getLong(c.getColumnIndex(KEY_ROWID))
            c.close()
        }
        return pill
    }
    ...
    val allPills: List<Any>
        ...
        get() {
            ...
            val c = db.rawQuery(dbPills, null)
            /** Loops through all rows, adds to list  */
            if (c.moveToFirst()) {
                do {
                    val p = Pills()
                    p.pillName = c.getString(c.getColumnIndex(KEY_PILLNAME))
                    p.pillId = c.getLong(c.getColumnIndex(KEY_ROWID))
                    pills.add(p)
                } while (c.moveToNext())
            }
            c.close()
    ...
    fun getAllAlarmsByPill(pillName: String): List<MedicineAlarm> {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                al.id = c.getInt(c.getColumnIndex(KEY_ROWID)).toLong()
                al.hour = c.getInt(c.getColumnIndex(KEY_HOUR))
                al.minute = c.getInt(c.getColumnIndex(KEY_MINUTE))
                al.pillName = c.getString(c.getColumnIndex(KEY_ALARMS_PILL_NAME))
                al.doseQuantity = c.getString(c.getColumnIndex(KEY_DOSE_QUANTITY))
                al.doseUnit = c.getString(c.getColumnIndex(KEY_DOSE_UNITS))
                al.dateString = c.getString(c.getColumnIndex(KEY_DATE_STRING))
                al.alarmId = c.getInt(c.getColumnIndex(KEY_ALARM_ID))
                alarmsByPill.add(al)
            } while (c.moveToNext())
        }
        c.close()
    ...
    fun getAlarmById(alarm_id: Long): MedicineAlarm {
        val dbAlarm = "SELECT * FROM " +
                ...
                KEY_ROWID + " = " + alarm_id
        val db = readableDatabase
        val c = db.rawQuery(dbAlarm, null)
        c?.moveToFirst()
        val al = MedicineAlarm()
        al.id = c.getInt(c.getColumnIndex(KEY_ROWID)).toLong()
        al.hour = c.getInt(c.getColumnIndex(KEY_HOUR))
    ...
    private fun combineAlarms(dbAlarms: List<MedicineAlarm>): List<MedicineAlarm> {
        ...
        for (al in dbAlarms) {
            if (timesOfDay.contains(al.stringTime)) {
                /** Add this db row to alarm object  */
                for (ala in combinedAlarms) {
                    if (ala.stringTime.equals(al.stringTime)) {
                        val day = getDayOfWeek(al.id)
                        val days: BooleanArray = ala.dayOfWeek
                        days[day - 1] = true
                        ala.dayOfWeek = days
                        ala.addId(al.id)
                    }
                ...
                newAlarm.pillName = al.pillName
                newAlarm.minute = al.minute
                newAlarm.hour = al.hour
                newAlarm.id = al.id
                newAlarm.dateString = al.dateString
                newAlarm.alarmId = al.alarmId
                val day = getDayOfWeek(al.id)
                days[day-1] = true
                newAlarm.dayOfWeek = days
                timesOfDay.add(al.stringTime)
    ...
    val history: List<Any>
        ...
        get() {
            ...
            val db = readableDatabase
            val c = db.rawQuery(dbHist, null)
            if (c.moveToFirst()) {
                do {
                    val h = History()
                    h.pillName = c.getString(c.getColumnIndex(KEY_PILLNAME))
                    h.dateString = c.getString(c.getColumnIndex(KEY_DATE_STRING))
                    h.hourTaken = c.getInt(c.getColumnIndex(KEY_HOUR))
                    h.minuteTaken = c.getInt(c.getColumnIndex(KEY_MINUTE))
                    h.doseQuantity = c.getString(c.getColumnIndex(KEY_DOSE_QUANTITY))
                    h.doseUnit = c.getString(c.getColumnIndex(KEY_DOSE_UNITS))
                    h.action = c.getInt(c.getColumnIndex(KEY_ACTION))
                    h.alarmId = c.getInt(c.getColumnIndex(KEY_ALARM_ID))
                    allHistory.add(h)
                } while (c.moveToNext())
            }
            c.close()
